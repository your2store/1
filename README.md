...
// Helper function to find the category of a product by its name
...
function calculateItemPrice(item) {
    ...
    // --- NEW RULES LOGIC START ---
    const category = findCategoryByProductName(item.name);
    ...
    if (data.method === 'per_unit') {
        ...
        // --- NEW RULE 1: Minimum 1m area for window price calculation ---
        let pricingArea = area;
        if (isWindow && pricingArea < 1) {
            pricingArea = 1;
        }
        basePrice = pricingArea * (data.price || 0);
        // --- END OF NEW RULE 1 ---
    }
    ...
    let shippingCost = area * (data.cbm || 0) * SHIPPING_RATE;

    // --- NEW RULE 2: Minimum shipping cost of 8 for windows ---
    if (isWindow && shippingCost < 8) {
        shippingCost = 8;
    }
    // --- END OF NEW RULE 2 ---
